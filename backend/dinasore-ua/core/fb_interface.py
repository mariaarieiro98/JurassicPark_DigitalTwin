...
class FBInterface:
    ...
    def avg_dist(self, _x, _y):
        ...
        distances = [self.distance(p1, p2) for p1, p2 in combinations(points, 2)]
        return sum(distances) / len(distances)

    def write_2_file(self, values, method_name, prediction_val):
        f = open("{0}{1}__{2}.txt".format(self.monitoring_path, self.fb_name, method_name), "a")
        f.write(str(values).strip('[]'))
        f.write(",{0}\n".format(prediction_val))
        f.close()
    ...
    def thread_monitoring_classify(self, _new_x):
        ...
        for neigh, name, algorithm in self.anomaly_algorithms:
            ...
            # For supervised algorithms that do have the "predict" function
                ...
                pca_output = self.pca.transform(self.scaler.transform(np.array(_new_x).reshape(1, -1)))
                prediction = algorithm.predict(pca_output)[0]

            pca_output = self.pca.transform(self.scaler.transform(np.array(_new_x).reshape(1, -1)))
            self.write_2_file(np.around(pca_output).tolist(), name, prediction)
            # update the actual state opc-ua variable
            self.actual_state = int(prediction)
    ...
    def thread_monitoring_pre_train(self):
        ...
        self.anomaly_algorithms[1][2] = DBSCAN(eps=self.avg_dist(result[:, 0], result[:, 1]),
                                               metric='euclidean', min_samples=2)
        self.anomaly_algorithms[2][2] = OneClassSVM(kernel='rbf', nu=self.contamination, gamma=0.05)

        ########################################################
        # Predict outliers - use DBSCAN (unsupervised technique) for first filtering of outliers
    ...
    def thread_monitoring_train(self):
        ...
        for i in np.arange(len(self.anomaly_algorithms)):
            ...
            open("{0}{1}__{2}.txt".format(self.monitoring_path, self.fb_name,  self.anomaly_algorithms[i][1]),
                 "w").close()
            for r in result:
                f = open("{0}{1}__{2}.txt".format(self.monitoring_path, self.fb_name, self.anomaly_algorithms[i][1]),
                         "a")
    ...
    def thread_monitoring_collect(self):
        ...
        ##############################################################
        # Collecting samples for training cycle
        while True:
            # Exit thread
            if self.stop_thread:
                break
            ...
            if not self.classify:
                ...
                if len(self.anomaly_data) == 0:
                    ...
                    for _, name, _ in self.anomaly_algorithms:
                        # just for visualization purposes - remove for accelerated processing
...
###############################################################################
    ...
    def __init__(self, fb_name, fb_type, xml_root, monitor=None):
        ...
        ###############################################################
        # variable to pass the actual state through opc-ua
        self.actual_state = 1
        ...
        if self.monitor_fb is not None:
            ...
            # Time lag like in predictive maintenance approaches
    ...
    def get_monitoring(self):
        ...
        diff_df_in_full = (df_in_full['time'] - df_in_full['time'].shift(1)).astype('<m8[ns]') / np.timedelta64(1, 's')
        diff_df_in_full = diff_df_in_full[1:]
        diff_df_in_half = (df_in_half['time'] - df_in_half['time'].shift(1)).astype('<m8[ns]') / np.timedelta64(1, 's')
        diff_df_in_half = diff_df_in_half[1:]
        diff_df_in_quarter = (df_in_quarter['time'] - df_in_quarter['time'].shift(1)).astype('<m8[ns]') / np.timedelta64(1, 's')
        diff_df_in_quarter = diff_df_in_quarter[1:]

        diff_df_out_full = (df_out_full['time'] - df_out_full['time'].shift(1)).astype('<m8[ns]') / np.timedelta64(1, 's')
        diff_df_out_full = diff_df_out_full[1:]
        diff_df_out_half = (df_out_half['time'] - df_out_half['time'].shift(1)).astype('<m8[ns]') / np.timedelta64(1, 's')
        diff_df_out_half = diff_df_out_half[1:]
        diff_df_out_quarter = (df_out_quarter['time'] - df_out_quarter['time'].shift(1)).astype('<m8[ns]') / np.timedelta64(1, 's')
